...
async def create_test_subscription(max_devices: int, hours: int, note: str = "") -> dict:
    """Создает тестовую подписку на N часов"""
    url = f"{WORKER_API_URL}/api/create"
    headers = {"Content-Type": "application/json"}
...
class AdminStates(StatesGroup):
    ...
    awaiting_test_user = State()  # Для тестовой подписки
...
def admin_menu_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton(text="🧪 Тестовая подписка", callback_data="admin_test")],  # НОВАЯ КНОПКА
...
async def show_main_menu(message: Message, user_id: int):
    ...
    try:
        ...
        tariff_name = next((t["name"] for t in TARIFFS.values() if t["max_devices"] == max_devices), f"{max_devices} устройства")
        if is_expired:
...
async def delete_device_callback(callback: CallbackQuery):
    user_id = callback.from_user.id
    _, hash_id, hwid = callback.data.split("_", 2)
    try:
...
async def admin_test_subscription(callback: CallbackQuery, state: FSMContext):
    ...
    # Создаем клавиатуру с выбором количества устройств
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
...
async def process_test_user(message: Message, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        # Создаем тестовую подписку на 1 час
...
async def admin_reply_callback(callback: CallbackQuery):
    ...
    if admin_id not in ADMIN_IDS:
        await callback.answer("⛔ Нет доступа", show_alert=True)
        return
    _, _, target_id = callback.data.split("_")
...
async def support_reply_callback(callback: CallbackQuery):
    user_id = callback.from_user.id
    _, target_user_id, admin_id = callback.data.split("_")
...
# =====================================================
# АДМИН-ПАНЕЛЬ (пользователи, статистика, логи)
# =====================================================
...
async def admin_test_user(callback: CallbackQuery, bot: Bot):
    """Быстрая тестовая подписка для конкретного пользователя"""
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("⛔ Нет доступа", show_alert=True)
        return
    ...
    try:
        # Создаем тестовую подписку на 1 час с 3 устройствами
