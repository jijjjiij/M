...
user_links = {}
user_hashes = {}
admin_reply_targets = {}  # admin_id -> user_id для ответа
...
async def show_main_menu(message: Message, user_id: int):
    ...
    try:
        ...
        # Пытаемся найти название тарифа, если не находим — используем по умолчанию
        tariff_name = None
...
async def admin_reply_callback(callback: CallbackQuery):
    ...
    # Формат: admin_reply_{admin_id}_{user_id}
...
async def support_reply_callback(callback: CallbackQuery):
    user_id = callback.from_user.id
    # Формат: support_reply_{target_user_id}_{admin_id}
