...
# =====================================================
# БАЗА ДАННЫХ (SQLite) — ТОЛЬКО ДЛЯ ПОЛЬЗОВАТЕЛЕЙ И ПЛАТЕЖЕЙ
# =====================================================

def init_db():
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    # Таблица пользователей (без подписок!)
    cur.execute("""
...
# =====================================================
# РАБОТА С API ВОРКЕРА (ГЕНЕРАЦИЯ ССЫЛОК)
# =====================================================

async def create_subscription(max_devices: int, note: str = "") -> dict:
    """Создает подписку через Worker API и возвращает ссылку"""
...
def admin_users_keyboard(users: list, page: int = 0) -> InlineKeyboardMarkup:
    ...
    for user in users[start:end]:
        user_id, username, privacy = user
        rows.append([
            InlineKeyboardButton(
                text=f"👤 {user_id} (@{username or 'no_name'})",
...
def device_selection_keyboard(subscription_url: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="💻 Windows", callback_data=f"device_windows_{subscription_url}"),
                InlineKeyboardButton(text="🍎 iPhone / iPad", callback_data=f"device_iphone_{subscription_url}"),
            ],
            [
                InlineKeyboardButton(text="🤖 Android", callback_data=f"device_android_{subscription_url}"),
                InlineKeyboardButton(text="🖥 macOS", callback_data=f"device_macos_{subscription_url}"),
            ],
            [
                InlineKeyboardButton(text="🐧 Linux", callback_data=f"device_linux_{subscription_url}"),
                InlineKeyboardButton(text="📺 TV", callback_data=f"device_tv_{subscription_url}"),
            ],
        ]
    )
...
def device_connection_text(device_key: str, subscription_url: str) -> str:
    ...
    return (
        ...
        f"🔗 **Ваша VPN-ссылка:**\n`{subscription_url}`\n\n"
        "⚠️ Ссылка активна для **указанного количества устройств**\n"
        ...
        if status == "paid":
            ...
            if subscription_url:
                ...
                await callback.message.edit_text(
                    text,
                    parse_mode="Markdown",
                    reply_markup=device_selection_keyboard(subscription_url)
                )
...
async def send_device_connection(callback: CallbackQuery):
    # Извлекаем device_key и subscription_url
...
async def admin_user_detail(callback: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="➕ Выдать подписку (3)",
                    callback_data=f"admin_grant_{target_id}_3",
                ),
                InlineKeyboardButton(
                    text="➕ Выдать подписку (5)",
                    callback_data=f"admin_grant_{target_id}_5",
                ),
            ],
...
async def admin_grant_quick(callback: CallbackQuery, bot: Bot):
    ...
    max_devices = int(parts[1]) if len(parts) > 1 else 3
    
    try:
        # Делаем запрос к Worker
        result = await create_subscription(max_devices, f"user_{target_id}")
        subscription_url = result.get("subscription_url")
        hash_id = result.get("hash")
        
        if subscription_url:
            log_admin_action(user_id, f"grant_{max_devices}_devices", target_id)
            
            await callback.answer(f"✅ Ссылка создана для {target_id} ({max_devices} устройств)", show_alert=True)
            ...
            await callback.message.edit_text(
                f"✅ **Подписка создана для пользователя {target_id}**\n\n"
...
async def admin_stats(callback: CallbackQuery):
    ...
    text = (
        ...
        f"📊 Всего платежей: (смотрите в логах)"
    )
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
...
async def process_grant_id(message: Message, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        max_devices = 5 if tariff_choice == "5" else 3
