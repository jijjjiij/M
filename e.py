...
# =====================================================
# ГЛОБАЛЬНЫЙ СЛОВАРЬ ДЛЯ ХРАНЕНИЯ ССЫЛОК ПОЛЬЗОВАТЕЛЕЙ
# =====================================================
...
class AdminStates(StatesGroup):
    awaiting_grant_id = State()      # ожидание ввода ID и тарифа
...
# Клавиатура для выбора устройства: передаем только device_key, ссылка будет взята из словаря user_links
...
async def check_payment(callback: CallbackQuery):
    ...
    try:
        ...
        if status == "paid":
            ...
            if subscription_url:
                # Сохраняем ссылку в словарь для этого пользователя
                user_links[user_id] = subscription_url
...
async def send_device_connection(callback: CallbackQuery):
    ...
    if not subscription_url:
        await callback.answer("❌ Ссылка не найдена. Попробуйте создать подписку заново.", show_alert=True)
        return
...
async def admin_grant_quick(callback: CallbackQuery, bot: Bot):
    ...
    try:
        ...
        if subscription_url:
            ...
            # Сохраняем ссылку в словарь для целевого пользователя
            ...
            # Отправляем сообщение пользователю с выбором устройства (без ссылки в кнопках!)
            try:
                text = (
