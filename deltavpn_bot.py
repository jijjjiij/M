...
# --- Оплата картой / крипто ---
@dp.callback_query(F.data.startswith("pay_card_"))
async def pay_card(callback: CallbackQuery, bot: Bot):
    ...
    try:
        ...
        if status == "paid":
            ...
            if subscription_url and hash_id:
                ...
                await bot.send_message(user_id, text, parse_mode="Markdown", reply_markup=device_selection_keyboard())
...
async def check_crypto(callback: CallbackQuery, bot: Bot):
    user_id = callback.from_user.id
...
async def my_subscription(callback: CallbackQuery, bot: Bot):
    ...
    try:
        ...
        await bot.send_message(user_id, text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb_buttons))
        try:
            await callback.message.delete()
        except:
            pass
...
async def add_device(callback: CallbackQuery, bot: Bot):
    user_id = callback.from_user.id
    hash_id = user_hashes.get(user_id)
    if not hash_id:
        await callback.answer("❌ У вас нет активной подписки", show_alert=True)
...
async def send_device_connection(callback: CallbackQuery, bot: Bot):
    user_id = callback.from_user.id
    ...
    try:
        ...
        return
    await bot.send_message(user_id, text, parse_mode="Markdown", reply_markup=delivered_connection_keyboard())
    try:
        await callback.message.delete()
    except:
        pass
...
async def delete_device_callback(callback: CallbackQuery, bot: Bot):
    user_id = callback.from_user.id
    ...
    try:
        ...
        if result.get("ok"):
            ...
            await my_subscription(callback, bot)
        else:
            await callback.answer(f"❌ Ошибка: {result.get('error')}", show_alert=True)
    except Exception as e:
        await callback.answer(f"❌ Ошибка: {str(e)}", show_alert=True)

# --- Удалить подписку (пользователь) ---
@dp.callback_query(F.data == "delete_my_subscription")
async def delete_my_subscription(callback: CallbackQuery, bot: Bot):
    user_id = callback.from_user.id
    hash_id = user_hashes.get(user_id)
    if not hash_id:
        await callback.answer("❌ У вас нет активной подписки", show_alert=True)
...
async def confirm_delete_subscription(callback: CallbackQuery, bot: Bot):
    user_id = callback.from_user.id
...
# --- Остальные колбэки (документы, поддержка, о нас) ---
@dp.callback_query(F.data == "documents")
async def show_documents_menu(callback: CallbackQuery, bot: Bot):
    await bot.send_message(callback.from_user.id, "📚 **Документы**", parse_mode="Markdown", reply_markup=documents_menu_keyboard())
    try:
        await callback.message.delete()
    except:
        pass
...
async def admin_broadcast(callback: CallbackQuery, bot: Bot):
    ...
    await bot.send_message(callback.from_user.id, "📢 **Рассылка**\n\nВыберите тип:", parse_mode="Markdown", reply_markup=kb)
    try:
        await callback.message.delete()
    except:
        pass
...
async def broadcast_all(callback: CallbackQuery, state: FSMContext, bot: Bot):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("⛔ Нет доступа", show_alert=True)
        return
    await state.update_data(broadcast_target="all")
    await bot.send_message(callback.from_user.id, "📢 **Введите текст сообщения для ВСЕХ пользователей:**", parse_mode="Markdown")
...
async def broadcast_user_list(callback: CallbackQuery, bot: Bot):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("⛔ Нет доступа", show_alert=True)
        return
    users = get_all_users()
    if not users:
        await bot.send_message(callback.from_user.id, "❌ Нет пользователей.", parse_mode="Markdown")
        try:
            await callback.message.delete()
        except:
            pass
    ...
    await bot.send_message(callback.from_user.id, f"👤 **Выберите пользователя:**\nВсего: {len(users)}", parse_mode="Markdown", reply_markup=kb)
    try:
        await callback.message.delete()
    except:
        pass
...
async def broadcast_user_select(callback: CallbackQuery, state: FSMContext, bot: Bot):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("⛔ Нет доступа", show_alert=True)
        return
    ...
    await bot.send_message(callback.from_user.id, f"📢 **Введите текст для пользователя {target_id} (@{user[1] or 'no_name'}):**", parse_mode="Markdown")
    await state.set_state(AdminStates.awaiting_broadcast_text)
    try:
        await callback.message.delete()
    except:
...
async def process_broadcast_text(message: Message, state: FSMContext, bot: Bot):
    ...
    await state.clear()
    await send_main_menu(bot, message.chat.id, admin_id)

# =====================================================
# ОБРАБОТКА ОТВЕТОВ (ПОЛЬЗОВАТЕЛЬ -> АДМИН)
# =====================================================
@dp.message(F.text)
...
async def admin_reply_callback(callback: CallbackQuery, bot: Bot):
    ...
    await bot.send_message(admin_id, f"✉️ **Ответ пользователю {target_id}**\n\nНапишите текст ответа:", parse_mode="Markdown")
    try:
        await callback.message.delete()
    except:
        pass
...
async def support_reply_callback(callback: CallbackQuery, bot: Bot):
    user_id = callback.from_user.id
    ...
    if user_id != target_user_id:
        await callback.answer("⛔ Это сообщение не для вас", show_alert=True)
        return
    await bot.send_message(user_id, "✉️ **Напишите ваш ответ:**", parse_mode="Markdown")
    try:
        await callback.message.delete()
    except:
        pass
...
async def admin_panel(callback: CallbackQuery, bot: Bot):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("⛔ Нет доступа", show_alert=True)
        return
    await bot.send_message(callback.from_user.id, "🔧 **Админ-панель**", parse_mode="Markdown", reply_markup=admin_menu_keyboard())
    try:
        await callback.message.delete()
    except:
        pass
...
async def admin_users(callback: CallbackQuery, bot: Bot):
    if callback.from_user.id not in ADMIN_IDS:
        return
    users = get_all_users()
    if not users:
        await bot.send_message(callback.from_user.id, "👥 Пользователей нет", parse_mode="Markdown")
        try:
            await callback.message.delete()
        except:
            pass
    ...
    await bot.send_message(callback.from_user.id, text, parse_mode="Markdown", reply_markup=admin_users_keyboard(users, 0))
    try:
        await callback.message.delete()
    except:
        pass
...
async def admin_users_page(callback: CallbackQuery, bot: Bot):
    if callback.from_user.id not in ADMIN_IDS:
        return
    ...
    if users:
        text = f"👥 **Все пользователи:** {len(users)}"
        await bot.send_message(callback.from_user.id, text, parse_mode="Markdown", reply_markup=admin_users_keyboard(users, page))
        try:
            await callback.message.delete()
        except:
            pass
...
async def admin_user_detail(callback: CallbackQuery, bot: Bot):
    if callback.from_user.id not in ADMIN_IDS:
        return
    ...
    await bot.send_message(callback.from_user.id, text, parse_mode="Markdown", reply_markup=kb)
    try:
        await callback.message.delete()
    except:
        pass
    ...
    try:
        ...
        if subscription_url and hash_id:
            ...
            await bot.send_message(callback.from_user.id,
                f"✅ Подписка создана для {target_id}\n📱 {max_devices} устройств\n📅 {days} дней",
            ...
            try:
                ...
                await bot.send_message(callback.from_user.id, f"⚠️ Не удалось отправить: {e}")
        else:
            await callback.answer("❌ Ошибка", show_alert=True)
    except Exception as e:
        await callback.answer(f"❌ Ошибка: {str(e)}", show_alert=True)
...
async def admin_grant(callback: CallbackQuery, state: FSMContext, bot: Bot):
    if callback.from_user.id not in ADMIN_IDS:
        return
    await bot.send_message(callback.from_user.id,
...
async def admin_test_subscription(callback: CallbackQuery, state: FSMContext, bot: Bot):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("⛔ Нет доступа", show_alert=True)
        return
    kb = InlineKeyboardMarkup(
...
async def test_select_devices(callback: CallbackQuery, state: FSMContext, bot: Bot):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("⛔ Нет доступа", show_alert=True)
        return
...
async def process_test_user(message: Message, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        if subscription_url and hash_id:
            ...
            await message.answer(f"✅ **Тестовая подписка создана!**\n\n👤 Пользователь: {target_id}\n📱 Устройств: {max_devices}\n⏱️ Срок: 1 час\n🔗 Ссылка: `{subscription_url}`")
            try:
                await bot.send_message(
                    target_id,
...
# --- Тестовая подписка для конкретного пользователя (быстрая) ---
    ...
    try:
        ...
        if subscription_url and hash_id:
            ...
            try:
                ...
                await bot.send_message(callback.from_user.id, f"⚠️ Не удалось отправить пользователю: {e}")
        else:
            await callback.answer("❌ Ошибка создания", show_alert=True)
    except Exception as e:
        await callback.answer(f"❌ Ошибка: {str(e)}", show_alert=True)

# --- Отправить сообщение пользователю (админ) ---
@dp.callback_query(F.data.startswith("admin_msg_"))
async def admin_msg_user(callback: CallbackQuery, state: FSMContext, bot: Bot):
    if callback.from_user.id not in ADMIN_IDS:
        return
    target_id = int(callback.data.replace("admin_msg_", ""))
    await state.update_data(broadcast_target=target_id)
    await bot.send_message(callback.from_user.id, f"📢 **Введите сообщение для пользователя {target_id}:**")
    await state.set_state(AdminStates.awaiting_broadcast_text)
    try:
        await callback.message.delete()
    except:
...
async def admin_stats(callback: CallbackQuery, bot: Bot):
    ...
    await bot.send_message(callback.from_user.id, text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Назад", callback_data="admin")]]))
    try:
        await callback.message.delete()
    except:
        pass
...
async def admin_logs(callback: CallbackQuery, bot: Bot):
    if callback.from_user.id not in ADMIN_IDS:
        return
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
...
# =====================================================
# ОБРАБОТЧИК НЕИЗВЕСТНЫХ КОЛБЭКОВ (защита от ошибок)
# =====================================================
@dp.callback_query()
async def catch_all_callbacks(callback: CallbackQuery):
    """Ловит все необработанные callback-запросы"""
    user_id = callback.from_user.id
    data = callback.data
    print(f"⚠️ Необработанный callback от {user_id}: {data}")
    await callback.answer("⏳ Эта кнопка устарела. Используйте /start", show_alert=True)
    await send_main_menu(callback.bot, user_id, user_id)
    try:
        await callback.message.delete()
    except:
        pass
